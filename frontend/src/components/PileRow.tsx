...
export default function PileRow({ piles, onTakeLeft, onTakeRight, gameActive }: PileRowProps) {
  ...
  return (
    <div className={`bg-hextech-panel/40 backdrop-blur-sm shadow-2xl border border-hextech-border min-h-[160px] w-full py-8 px-4 flex items-center relative overflow-hidden ${piles.length === 1 ? 'justify-center' : 'justify-between'}`}>
      ...
      {middlePiles.length > 0 ? (
        <div className="flex-1 relative mx-4 min-w-0 flex items-center h-full">
          ...
          <div className="overflow-x-auto custom-scrollbar flex items-center w-full px-2 py-4">
            <div className="flex flex-nowrap items-center min-w-max gap-4 mx-auto w-fit px-8">
