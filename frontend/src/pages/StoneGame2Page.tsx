...
export default function StoneGame2Page() {
  ...
  useEffect(() => {
    ...
    let active = true;
    analysisService.analyze('stone-game-2', gameState, difficulty).then((res: any) => { if (active) setPrediction(res); });
    return () => { active = false; };
  ...
  const handleTake = (count: number) => {
    if (gameState && !gameState.gameOver) {
      setGameState(applyMoveWithExplanation(engine, gameState, count, 'stone-game-2', prediction || undefined));
    }
  };
  ...
  return (
    <GameLayout title="Stone Game II">
      {!gameState ? (
        <GameSetup
